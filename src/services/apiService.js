...
export const speechToText = async (audioBlob, sourceLanguage = LANGUAGES.ENGLISH) => {
  try {
    ...
    console.log("Transcript field:", data.transcript)
    ...
    if (!transcribedText || transcribedText.trim() === '') {
      console.error("Empty transcript received:", data)
      console.error("Audio was processed but no speech detected. This could be due to:")
      console.error("1. Speaking in wrong language (current:", sourceLanguage, ")")
      console.error("2. Audio too quiet (try speaking louder)")
      console.error("3. Background noise interference")
      throw new Error(`No speech detected. Current language: ${sourceLanguage}. Try speaking louder in the selected language.`)
    }
